...
gulp.task('js', function() {
  return gulp.src('./app/js/script.js')
    .pipe(gulp_plumber({errorHandler: gulp_notify.onError("JS Error: errorHandler: err.message")}))
...
gulp.task('browser-sync', function() {
  browserSync.init({
    proxy: "localhost", // !!!!! Replace localhost if you need
    notify : false,
  });
});
...
gulp.task( 'watch-browser', function()
  ...
  gulp.watch( './index.php', ['browser-update'] ); // Update of browser-sync after php modification
} );

gulp.task( 'default', [ 'css', 'prefixer', 'js', 'watch' , 'imagemin'] );
gulp.task( 'share', [ 'css', 'prefixer', 'js', 'browser-sync', 'watch-browser', 'imagemin' ] );